...
import transformers
import joblib
from artifact.bertutils import *
from artifact.anaphora_utils import *   
...
def extract_LF(Data,nlp1):
    ...
    Data['numPrecedingNPsPara']=Data.apply(lambda x: numPrecedingNPsPara(x['Pronoun'],x['sents']), axis=1)
    ...
    Data['numNPsInPara']=Data.apply(lambda x: numNPsInPara(x['sents']), axis=1)
...
def getprediction(test,y_pred,idx,Wy,Set):
    ...
    for k,v in di.items():
        ...
        if (QNY[2]==1 and QNY[0]==0) or (QNY[2]==1 and Yw==0):
            ...
            #Ambiguous
            dd[k]="Ambiguous"
    newdf = Set[Set['Id'].isin(idx[test])][["Id","Context","Pronoun","Position"]]
    newdf.drop_duplicates(inplace=True)
    newdf['Detected As']=newdf.apply(lambda x: dd[x['Id']] ,axis=1)
    return newdf
